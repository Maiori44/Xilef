    ...
    return new Discord.MessageEmbed()
      ...
      .setDescription(
        '```lua\ndepth: ' +
          this.depth +
          '\ntier: ' +
          EconomySystem.driller +
          '\ncash found: ' +
          this.cash +
          '\nhealth: ' +
          this.hp +
          '/' +
          100 * EconomySystem.driller +
          '```'
      )
      .setTimestamp();
  }
...
Driller = new Game((EconomySystem) => {
  return new DrillerGame(EconomySystem);
});
Driller.Ores = [
...
Commands.driller = new Command(
  'Dig deeper and deeper to find the treasures\n\n' + Driller.help,
  (message, args) => {
    ...
    switch (args[0]) {
      ...
      case 'dig': {
        if (Driller.Ores[DrillerGame.depth].tier > EconomySystem.driller) {
          ...
          InfoEmbed.addField(
            'Latest event:',
            'Your driller is too weak to dig any further!'
          );
        ...
        if (hurtchance <= Driller.Ores[DrillerGame.depth].lavachance) {
          msg =
            'Your driller digs deeper..and finds lava! Your driller got damaged!';
          DrillerGame.hp = DrillerGame.hp - Math.max(7 * DrillerGame.depth, 1);
          DrillerGame.hitlava = true;
        } else {
          msg =
            'Your driller digs deeper..and finds ' +
            Driller.Ores[DrillerGame.depth].name +
            '! (worth ' +
            Driller.Ores[DrillerGame.depth].value +
            ')';
          DrillerGame.cash =
            DrillerGame.cash + Driller.Ores[DrillerGame.depth].value;
          DrillerGame.depth++;
          if (Driller.Ores[DrillerGame.depth].tier > EconomySystem.driller) {
            msg =
              msg +
      ...
      case 'repair': {
        ...
        if (DrillerGame.hp == 100 * EconomySystem.driller) {
          ...
          EconomySystem.buy(
            cost,
            message,
            'Your driller recovered ' +
              cost +
              ' hp! (' +
              cost +
              ' DogeCoins spent)',
            'You need ' +
              (cost - EconomySystem.money) +
              ' more DogeCoins for this.'
          )
        ) {
          DrillerGame.hp = Math.min(
            DrillerGame.hp + cost,
            100 * EconomySystem.driller
          );
        }
      ...
      case 'upgrade': {
        if (EconomySystem.driller == 29) {
          ...
          EconomySystem.buy(
            Driller.tiers[EconomySystem.driller - 1],
            message,
            'Your driller reached tier ' +
              (EconomySystem.driller + 1) +
              '! (' +
              Driller.tiers[EconomySystem.driller - 1] +
              ' DogeCoins spent)',
            "You don't have enough DogeCoins to upgrade your driller (" +
              Driller.tiers[EconomySystem.driller - 1] +
              ' DogeCoins needed)'
          )
        ) {
      ...
      case 'cashin': {
        message.channel.send(
          'Your driller comes back, and gives you all the DogeCoins it had collected.'
        );
    ...
    if (DrillerGame.hp < 1) {
      message.channel.send(
        'Your driller broke! It lost whatever it had collected.'
      );
  ...
  'Game',
  [
    new RequiredArg(0, Driller.help, 'command'),
